...
//	    mysql_set_charset( DB_CHARSET, $this->db );
	    ...
	    $this->sql_string = 'SHOW TABLES';

	    $this->executeQuery($this->sql_string);
	    ...
	    while ( $r = $this->query->fetch(PDO::FETCH_NUM)) {
	...
	private function make_sql( $sql_file, $table ) {
	    ...
	    // Get table structure
	    $this->sql_string = 'SHOW CREATE TABLE ' . $this->sql_backquote( $table );
	    ...
	    if ($this->executeQuery($this->sql_string)) {

	    	if ($this->query->rowCount() > 0 ) {
	    		$sql_create_arr = $this->query->fetch($PDO::FETCH_OBJ);
